        ...
        def insertStep(node):
            if not self.rightWay:
                ...
                for i in self.rightWay:
                    if i.rightNode != 0:
                        # print('right', '[', node.x, node.y, '] = ',
                        #       '[', i.rightNode.x, i.rightNode.y, ']')
                    ...
                    if i.leftNode != 0:
                        # print('left', '[', node.x, node.y, '] = ',
                        #       '[', i.leftNode.x, i.leftNode.y, ']')
                    ...
                    if i.topNode != 0:
                        # print('top', '[', node.x, node.y, '] = ',
                        #       '[', i.topNode.x, i.topNode.y, ']')
                    ...
                    if i.bottomNode != 0:
                        # print('bottom', '[', node.x, node.y, '] = ',
                        #       '[', i.bottomNode.x, i.bottomNode.y, ']')
        ...
        def findWay(goal):
            ...
            while True:
                ...
                if obstacles['left']:
                    ...
                    if insertOpen(Node(currentTrainer.x - 1, currentTrainer.y, g_n_ + h_n_)):
                ...
                if obstacles['right']:
                    ...
                    h_n_ = manhatan(currentTrainer.x + 1,
                                    currentTrainer.y, goal)
                    if insertOpen(Node(currentTrainer.x + 1, currentTrainer.y, g_n_ + h_n_)):
                        currentNode.rightNode = Node(currentTrainer.x + 1,
                ...
                if obstacles['up']:
                    ...
                    if insertOpen(Node(currentTrainer.x, currentTrainer.y - 1, g_n_ + h_n_)):
                        currentNode.topNode = Node(currentTrainer.x, currentTrainer.y - 1, g_n_ + h_n_)
                if obstacles['down']:
                    g_n_ = manhatan(currentTrainer.x,
                                    currentTrainer.y + 1, start)
                    h_n_ = manhatan(currentTrainer.x,
                    ...
                    if insertOpen(Node(currentTrainer.x, currentTrainer.y + 1, g_n_ + h_n_)):
                        currentNode.bottomNode = Node(
                            currentTrainer.x, currentTrainer.y + 1, g_n_ + h_n_)
                insertStep(currentNode)

                print('F[', currentNode.x, currentNode.y, ']')
                if currentNode.leftNode != 0:
                    print('    L[', currentNode.leftNode.x, currentNode.leftNode.y, ']')
                if currentNode.bottomNode != 0:
                    print('    B[', currentNode.bottomNode.x, currentNode.bottomNode.y, ']')
                if currentNode.rightNode != 0:
                    print('    R[', currentNode.rightNode.x, currentNode.rightNode.y, ']')
                if currentNode.topNode != 0:
                    print('    T[', currentNode.topNode.x, currentNode.topNode.y, ']')
                

                if currentTrainer.x == goal.x and currentTrainer.y == goal.y:
                    for k in self.rightWay:
                        print('Paso', str(self.rightWay.index(k)), '[', k.x, k.y, ']')
                    return self.rightWay

        # print(pokemon)
        for i in range(len(pokemon)):
            print('----------------------------------Pokemon ', str(i), '----------------------------------')
            self.steps.append(findWay(pokemon[i]))
...
class Tablero:
    def __init__(self, size):
        ...
        self.tablero = []
        self.pokemonArray = []
        self.trainer = Trainer(randint(0, self.size - 1), randint(
            0, self.size - 1), self.window, 'pokeball2')
    ...
    def findPokemon(self):
        ...
        def Move(trainer):
            ...
            self.trainer = Trainer(self.a.steps[i][j].y, self.a.steps[i][j].x, self.window, 'pokeball2')

            
            self.tablero[self.a.steps[i][j - 1].y][self.a.steps[i][j - 1].x].label.grid(column=self.a.steps[i][j - 1].x, row=self.a.steps[i][j - 1].y)
            self.trainer.label.grid(column=self.trainer.x, row=self.trainer.y)
            if i == len(self.a.steps[j]):
                j += 1
        ...
        Move(self.trainer)
        self.window.after(1000, self.findPokemon)
